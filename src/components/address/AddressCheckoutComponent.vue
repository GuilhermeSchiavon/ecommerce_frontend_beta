<template>
    ...
    <section v-else class="max-w-4xl w-full mx-auto">
        ...
        <form v-show="showEdit">
            <div v-if="!ocultarCamps" class="grid grid-cols-1 gap-6 mt-2 sm:grid-cols-3">                        
            ...
            <div v-if="!ocultarCamps" class="grid grid-cols-1 gap-6 mt-2 sm:grid-cols-2">
                <div>
            ...
            <div v-if="!ocultarCamps" class="mt-2">
        ...
        <main class="max-h-40 overflow-y-auto overflow-x-hidden">
            <article v-for="address in addresses" :key="address.id" :class="{'border-gray-400 border-2 ' : address.default }" class="relative group mb-1 bg-white border p-3 sm:p-3 shadow-sm transition ">
                <div @click="newAddress = false,  newObjAddress = address, validarAddress()" class="flex justify-between cursor-pointer">
                    <h3 class="mt-0.5 font-normal text-gray-900">
                        {{address.street}}, {{ address.number }} - {{ address.complement }}
                    </h3>
                </div>
                <div @click="newAddress = false,  newObjAddress = address, validarAddress()" class="flex justify-between items-center cursor-pointer">
                ...
                <div class="absolute right-4 top-1/2 flex -translate-y-1/2 gap-2">
                    <svg @click="updateAddress(address)" class="w-4 h-4 md:w-5 md:h-5 text-gray-300 hover:text-blue-600 cursor-pointer" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                    </svg>
                    <span @click="newAddress = false,  newObjAddress = address, validarAddress()" :class="{ 'border-gray-700' : address.default }" class="box-content hidden sm:block h-2 w-2 rounded-full border-[5px] border-gray-300 bg-white cursor-pointer"></span>
                </div>
            </article>
        </main>
    </section>
