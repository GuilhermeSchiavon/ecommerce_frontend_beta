<template>
    <section class="__reviews w-full px-6 py-4 mx-auto bg-white dark:bg-gray-800">  
        <div v-if="reviews.length > 0" class="container pb-4 border-b mb-8">
            <div v-for="(review, index) in reviews" :key="index" class="_reviewr py-4 dark:bg-gray-800">
                ...
                <div class="ml-4 space-y-1">
                    ...
                    <p class="text-xs text-gray-600 dark:text-gray-400"> {{ new Date(review.createdAt).toLocaleString("pt-BR") }}</p>
        ...
        <form  @submit.prevent="rate()" class="mt-0 container">
            <p class="mt-3 font-semibold mb-4 text-gray-600 dark:text-gray-400">Adicionar Avaliação</p>
            <RateComponent @ratings="ratings"/>
            <div class="w-full mt-6">
                <label class="block mb-2 text-sm font-medium text-gray-600 dark:text-gray-200">Comentário</label>
            ...
            <div class="flex lg:block justify-center mt-6">
                <button class="px-8 text-sm py-4 text-white transition-colors duration-200 transform bg-gray-900 rounded-md hover:bg-gray-800 focus:outline-none focus:bg-gray-600">Avaliar</button>
            </div>
        </form>
    </section>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            ...mapMutations(['SET_ALERT', 'REDIRECT']),
            ratings(rate){
                this.rating = rate
            },
            rate(){
                ...
                } else this.addReview({id: this.$route.params.id, comment: this.comment, rating: this.rating})
            }
        }
    }
</script>
